...
const DessertMenu = () => {
  ...
  const { desserts, loading, error } = useSelector((state) => state.menu);

  useEffect(() => {
    dispatch(fetchMenu()); // Fetch all menu items from backend
  }, [dispatch]);

  if (loading) return <p>Loading dessert menu...</p>;
          ...
          {desserts.map((item) => (
            <div key={item._id} className="menu-list-card">
              ...
              <div className="menu-list-details">
                ...
                <div className="menu-list-comments">
